...
def get_results(Para, v0, CD8Tm, Bm, A, CD4Tm=0.02):
    ...
    if A != 0:
        initial = [0.01, 0, Para[52] / Para[62], 0, 0, Para[53]/Para[65], 0, 0, Para[159]/2, 0, 0, 0, 0, 0, 0,
                   ...
                   0, 0]
        Ig0 = (1 + Para[51] * initial[25] / (initial[25] +
                                             Para[116])) * Bm * Para[100] / Para[107]
        initial[31] = A
        initial[30] = Ig0
        initial[23] = Bm
        initial[20] = CD8Tm
...
def get_Bm(Ab,Para):#calculate corresponding Bm based on a given Ab level
    initial = [0.01, 0, Para[52] / Para[62], 0, 0, Para[53] / Para[65], 0, 0, Para[159] / 2, 0, 0, 0, 0, 0, 0,
               Para[54] / Para[70], 0, Para[160] / 2, 0, 0, 0, 0, 0, 0, Para[77] /
               Para[101], Para[82] / Para[102],
               Para[84] / Para[103], (Para[88] + Para[54] / Para[70] * Para[90]) /
...
# ----------- Sampling Trajectories with different initial memory cells ---------------
    try:
        data = np.load('Bsingle_M%i_timecourse_spl.npy' %
        ...
        B_Mem = np.random.uniform(0,get_Bm(1500,Parameter), M)
        ...
        args = []
        for j in range(M):
            args.append([Parameter, v0, T_Mem[j], B_Mem[j], A, 0.2])
        count = 0
        ...
        np.save('Bsingle_M%i_timecourse_spl.npy' % mode, data)
...
# -------------------- Protection border in the Bm, CD8+Tm space ----------------------------------
    ...
    axbt.set_xticklabels([0, 500, 1000])
    axbt.set_xlabel('Ab (t$^*$)')
    axbt.set_ylabel('CD8+T$_M$ (t$^*$)')
    bbox = dict(boxstyle="square", fc="w", color='k')
    axbt.text(x=1200,y=0.4,s='ss',bbox=bbox,ha='right')
    plt.tick_params(zorder=100)
    figbt.subplots_adjust(bottom=0.25,left=0.25)

    # Saving figures
    figbt.savefig("BFig_BmTm_GeoMode%i.png" % mode, dpi=300)
    fig.savefig("BFig_type_M%i.png" % mode, dpi=300)
    
    figbt.savefig("BFig_BmTm_GeoMode%i.svg" % mode, dpi=300)
    fig.savefig("BFig_type_M%i.svg" % mode, dpi=300)
